...
class DbManager{
    ...
    fun getAllAdsFirstPage(filter: String, readDataCallback: ReadDataCallback?){
        val query = if(filter.isEmpty()){
            db.orderByChild(FILTER_WAY).limitToLast(ADS_LIMIT)
        } else {
    ...
    fun getAllAdsNextPage(time: String, filter: String, readDataCallback: ReadDataCallback?){
       if(filter.isEmpty()){
            val query = db.orderByChild(FILTER_WAY).endBefore(time).limitToLast(ADS_LIMIT)
            readDataFromDb(query, readDataCallback)
       } else {
    ...
    private fun getAllAdsByFilterNextPage(tempFilter: String, time: String, readDataCallback: ReadDataCallback?): Query{
        val orderBy = tempFilter.split("|")[0]
        val filter = tempFilter.split("|")[1]
        val query = return db.orderByChild("/adFilter/$orderBy")
    ...
    fun getAllAdsFromCatFirstPage(cat: String, filter: String, readDataCallback: ReadDataCallback?){
        val query = if(filter.isEmpty()) {
            db.orderByChild(FILTER_CAT_WAY)
    ...
    fun getAllAdsFromCatNextPage(cat: String, time: String, filter: String, readDataCallback: ReadDataCallback){
        if(filter.isEmpty()){
            val query = db.orderByChild(FILTER_CAT_WAY)
    ...
    companion object{
        ...
        const val FILTER_WAY = "/adFilter/time"
        const val FILTER_CAT_WAY = "/adFilter/cat_time"
        const val INFO_NODE = "info"
        const val MAIN_NODE = "main"
        const val FAVS_NODE = "favs"
        const val ADS_LIMIT = 100
